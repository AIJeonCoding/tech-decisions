...
const PAYMENT_ARTICLES: SeedArticle[] = [
  ...
  },
  {
    ...
    summary: '계정계와 Oslo의 이중 개발 + 동일 기능 API 병행 운영 + 데이터 정합성 검증',
    bodyMd: '카카오뱅크 Oslo Project는 계정계 시스템 장애 시에도 모바일 앱 첫 화면이 동작하도록 만든 "장애 격리 시스템"이다. 핵심은 "전체 계좌 목록 불러오기" 기능을 계정계와 Oslo 두 시스템에서 동일하게 제공하는 API로 병행 운영하면서 데이터 정합성을 검증하는 구조다. 원본 시스템(계정계)에서 Near Real Time으로 계좌 정보·잔액을 Oslo로 복제하고, 두 시스템의 결과를 지속적으로 비교해 차이를 추적했다. "계정계와 Oslo의 이중 개발 및 데이터 정합성 검증 프로세스"라는 표현은 신구 시스템 마이그레이션에서 표준이 되어가는 dual-run/shadow-mode 패턴의 은행권 적용 사례를 보여준다. 결과적으로 첫 화면 안정성을 계정계 장애로부터 분리시켜 모바일 뱅킹 가용성을 끌어올렸다.',
    publishedAt: '2025-05-16',
    domains: ['payment-settlement', 'msa-migration'],
    tags: ['ledger', 'concurrency', 'idempotency', 'msa'],
  },
  {
    companySlug: 'kakaobank',
    url: 'https://tech.kakaobank.com/posts/2412-financial-batch-system-with-airflow/',
    title: 'Airflow로 펀드 배치 시스템 완벽 구축하기',
    summary: 'Airflow 기반 펀드 결산 배치 + 감사로그·접근통제로 규제 준수와 무결성 보장',
    bodyMd: '카카오뱅크는 클라우드 환경에 적합한 배치 시스템이 없어 Airflow 기반으로 펀드 배치를 직접 구축했다. "연 1회 수행되는 결산이라는 업무를 통해 펀드의 수익을 분배하고 기준가를 초기화"하는 핵심 업무를 자동화 대상으로 삼았다. 설계 원칙은 "복잡한 비즈니스 로직을 파이썬 코드로 옮겨 다시 작성하게 되면, 테스트가 어려워지고 로직이 여기저기 분산됨에 따라 업무의 일관성과 무결성을 보장할 수 없게 된다"는 진단에서 출발해, 비즈니스 로직은 기존 도메인 코드에 남기고 Airflow는 오케스트레이션만 담당하도록 분리했다. 감사 로그·접근 통제·재실행 안전성 같은 금융 규제 요건을 Airflow DAG 위에서 표준화해, "정확성+자동화"를 동시에 만족하는 결산 파이프라인을 완성했다.',
    publishedAt: '2024-12-19',
    domains: ['payment-settlement', 'realtime-data'],
    tags: ['settlement', 'ledger', 'data-pipeline'],
  },
  {
    companySlug: 'kakaobank',
    url: 'https://tech.kakaobank.com/posts/2501-card-openbanking-migration-project/',
    title: '카드 청구금액 알림 서비스의 홀로서기 대모험',
    summary: '오픈뱅킹 배치성 서비스를 코어로부터 분리해 독립 운영 + 단계적 마이그레이션',
    bodyMd: '카카오뱅크의 카드 청구금액 알림 서비스는 그간 오픈뱅킹 코어와 강결합되어 있었다. 배치성 트래픽이 오픈뱅킹 본 트래픽에 영향을 주는 구조여서, 신규 청구금액 알림 시스템을 별도 도메인 서비스로 분리하고 코어 데이터에 대한 의존을 최소화하는 인터페이스로 재설계했다. 마이그레이션은 신구 시스템 병행 운영 → 일부 트래픽 전환 → 전체 전환의 단계적 방식으로 진행해, 운영 영향 없이 배치 도메인을 분리해냈다. 결과적으로 청구금액 알림은 별도의 배치 스케줄과 장애 격리 단위를 가지게 됐고, 오픈뱅킹 본 채널은 응답성을 회복했다.',
    publishedAt: '2025-01-03',
    domains: ['payment-settlement', 'msa-migration'],
    tags: ['settlement', 'msa', 'monolith-decomposition'],
  },
];
...
const PLATFORM_ARTICLES: SeedArticle[] = [
  ...
  },
  {
    ...
    summary: '프라이머리/코어/세컨더리 3계층 멀티모듈 + 선택적 빌드로 4개 서비스 스케일아웃 분리',
    bodyMd: '카카오뱅크 메시지 허브는 단일 헥사고날로 묶여 있던 4개 서비스(api, data-api, loader, consumer)가 모두 같은 데이터베이스에 접근하고 같은 이벤트 브로커 메시지를 중복 수신하는 문제를 겪고 있었다. 해법은 "메시지허브에 어울리는 헥사고날로 만들어보자"는 원칙 아래 한 프로젝트에서 소스를 공유하면서도 4개 서비스가 각자 다른 어댑터 조합을 갖는 멀티모듈 헥사고날을 설계한 것. 모듈 구조는 ① 프라이머리 어댑터 모듈(서비스별) ② 코어 모듈(비즈니스 로직 + 포트 인터페이스) ③ 세컨더리 어댑터 모듈(Redis, JPA, Kafka 구현체)로 분리했고, 각 프라이머리 어댑터는 필요한 코어와 세컨더리만 선택적으로 의존성에 추가한다. @ConditionalOnExpression과 서비스명 기반 빌드 타임 제거로 "금지할 모듈을 사전에 제거"하는 안정성 전략을 채택했다. 결과적으로 도메인 로직 중복 없이 4개 서비스가 독립적으로 스케일아웃 가능한 구조를 얻었다.',
  ...
  },
  {
    ...
    summary: 'MSA 풀스택 전환 전 단계로 Spring Modulith를 일부 채택해 모듈 경계만 먼저 강제',
    bodyMd: '카카오뱅크는 모놀리스에서 MSA로 한 번에 가지 않고, Spring Modulith의 모듈 경계 강제 기능만 먼저 도입했다. ApplicationModule 어노테이션과 의존성 규칙 검증으로 "도메인 간 직접 의존"을 컴파일 타임에 차단해, 향후 MSA로 분리할 때 모듈을 그대로 떼어낼 수 있는 경계를 미리 정립했다. Modulith의 모든 기능을 통째로 받아들이는 대신 필요한 부분만 "체리픽"해 도입했다는 점이 핵심으로, MSA 전환을 빅뱅이 아닌 점진적 진화로 만드는 실용적인 전략이다.',
  ...
  },
  {
    ...
    summary: 'Consul 서비스 레지스트리 + 동적 라우팅으로 멀티 데이터센터 무중단 확장',
    bodyMd: '카카오뱅크는 모놀리식에서 마이크로서비스로 전환하며 서버 증설 시마다 수동으로 설정을 변경하는 운영 부담을 없애기 위해 Consul 기반 서비스 디스커버리를 도입했다. 핵심은 멀티 데이터센터 환경에서 안정적 확장이 필수였고, 서비스 메시 없이도 동적 라우팅과 헬스체크를 운영팀이 직접 관리할 수 있어야 했다는 점이다. "기존 시스템을 새로운 시스템으로 전환해나가는 과정에서 연속성을 유지할 수 있도록 많은 노력을 기울이고 있으며, 이에 서비스 디스커버리가 핵심적인 역할을 담당하고 있습니다"라는 표현은 은행 환경에서 정확성·안정성을 양보하지 않으면서 자동화·확장 가능한 아키텍처를 받아들이는 카카오뱅크의 태도를 단적으로 보여준다.',
    publishedAt: '2025-02-15',
    domains: ['msa-migration', 'realtime-data'],
    tags: ['msa', 'observability', 'sre'],
  },
];
