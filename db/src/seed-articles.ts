...
const PAYMENT_ARTICLES: SeedArticle[] = [
  {
    ...
    summary: '결제 API 멱등성을 Idempotency-Key + Redis 12시간 TTL로 구현',
    bodyMd: '결제 요청마다 클라이언트가 발급한 Idempotency-Key를 받아 Redis에 일정 시간 저장하고, 같은 키가 들어오면 이전 응답을 그대로 반환한다. 이를 통해 네트워크 재시도로 인한 중복 결제를 방지하고, 12시간 TTL로 메모리 사용량을 제한한다.',
    publishedAt: '2024-08-12',
    domains: ['payment-settlement'],
    tags: ['payment', 'idempotency', 'concurrency', 'redis'],
  },
  {
    companySlug: 'toss',
  ...
  },
  {
    ...
    title: '대사 시스템',
    summary: '이중기장 + PG 전문 일일 리컨실로 외부와 내부의 차이를 자동 검출',
    bodyMd: '내부 원장은 차변/대변 한 쌍의 분개로 기록하고, PG에서 받은 정산 전문과 매일 자정 비교하여 차이를 알람으로 띄운다. 자동 매칭 실패 건은 운영자 콘솔에서 직접 처리할 수 있도록 워크플로우가 분리되어 있다.',
    publishedAt: '2024-04-20',
    domains: ['payment-settlement'],
    tags: ['settlement', 'ledger', 'payment'],
  },
  {
    companySlug: 'toss',
  ...
  },
  {
    ...
    summary: 'Choreography Saga + 보상 액션 역순 실행',
    bodyMd: '여러 마이크로서비스가 관여하는 결제 흐름은 Choreography Saga로 모델링하고, 실패 시 각 단계의 보상 액션을 역순으로 실행한다. 각 서비스는 자신의 보상 책임만 알면 되어 결합도가 낮다.',
  ...
  },
  {
    ...
    title: '동시 주문 처리 이야기',
    summary: 'Redis Redlock으로 사용자 단위 락 짧게 잡기',
    bodyMd: '같은 사용자가 빠르게 중복 결제하는 케이스를 막기 위해 Redis Redlock으로 사용자 단위 락을 짧게 잡는다. 락 타임아웃과 자동 해제로 데드락을 방지한다.',
  ...
  },
  {
    ...
    title: 'Outbox 패턴 도입기',
    summary: 'DB outbox + 별도 워커가 메시지 발행으로 트랜잭션 안전성',
    bodyMd: '결제 완료 이벤트를 DB outbox 테이블에 같은 트랜잭션으로 기록하고, 별도 워커가 메시지 브로커로 발행하며 실패 시 자동 재시도한다. 메시지 발행 실패가 거래 원자성을 깨지 않는다.',
    publishedAt: '2024-11-05',
    domains: ['payment-settlement'],
    tags: ['payment', 'kafka', 'msa'],
  },
  {
    companySlug: 'woowahan',
